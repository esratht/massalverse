...
// --- ZODIAC GÖLGE VERİ TABANI (Manifesto için entegre edildi) ---
const ZODIAC_DATA = {
    // 
...
// --- YENİ METRİK: SİSTEM BASKISI KODU (System Pressure Code) ATAMA FONKSİYONU ---
function calculateSystemPressure(name, date, time) {
    
    // Doğum verilerini işle
    const dateObj = new Date(date);
    const month = dateObj.getMonth() + 1; // 1-12
    const day = dateObj.getDate(); // 1-31
    const hour = parseInt(time.split(':')[0]);
    
    const planets = ["GÜNEŞ", "AY", "MERKÜR", "VENÜS", "MARS", "JÜPİTER", "SATÜRN"];
    let report = {};

    // A. STELYUM SENSÖRÜ (ANA ODAK) DÜZELTİLDİ: Güneş Burcu kullanılıyor
    ...
    // C. ZARAR VURUCU GÜÇ (ZAYIFLIK)
    const weaknessCount = Math.floor(month / 4) + (day % 3); 
    
    let weakPlanets = [];
    for (let i = 0; i < weaknessCount; i++) {
        const randomPlanetIndex = Math.floor(Math.random() * planets.length);
        const weakPlanet = planets[randomPlanetIndex];
        
        if (!weakPlanets.includes(weakPlanet)) {
            weakPlanets.push(weakPlanet);
        }
    }
...
async function sendMessage() {
    ...
    if (step === 0) {
        showTyping();
        // CSP UYUMLU: Fonksiyon referansı kullanıldı
        setTimeout(() => {
    ...
    try {
        // API isteğinde tüm userData'yı gönder
        ...
        removeTyping();
        addMessage("Okyanus çok dalgalı, sesini duyamıyorum... (Bağlantı Koptu)", 'ai');
    }
}
...
function showBerat(beratData) {
    ...
    // BURCUN GÖLGE ARKETİPİ GÖSTERİLİYOR
    ...
    // SİSTEM BASKISI KODLARI (HTML'deki yerleri kontrol edilmeli)
